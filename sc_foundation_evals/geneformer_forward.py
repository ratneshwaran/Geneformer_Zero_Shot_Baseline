...
class Geneformer_instance():
    def __init__(self,
        ...
        # Generate run_id for output directory
        self.run_id = f'{self.model_run}__{os.path.basename(saved_model_path) if saved_model_path else "default"}'
    ...
    def load_tokenized_dataset(self,
        ...
        try:
            ...
            if os.path.exists(dataset_path):
                ...
                self.tokenized_dataset = Dataset.load_from_disk(dataset_path, keep_in_memory=False)
            else:
    ...
    def tokenize_data(self,
        ...
        # initialize tokenizer with smaller chunk size to avoid memory issues
        self.tokenizer = TranscriptomeTokenizer(
            cols_to_keep,
            nproc=self.num_workers,
            keep_counts=True,
            # Use a smaller chunk size to reduce peak RAM during tokenization
            chunk_size=128,
        )
        ...
        self.tokenizer.tokenize_data(adata_dir,
                                     ...
                                     dataset_name,
                                     file_format=ext,
                                     use_generator=True)  # Force generator method
